...
// ---------------------------------------------------------------------------
// Main loop  (direct port of Python DeviceWorker.run)
// ---------------------------------------------------------------------------

void DeviceWorker::run() {
    logInfo(config_.name, "Worker started (" + config_.type + " @ " +
            config_.tuya_config.ip + ")");
...
// ---------------------------------------------------------------------------
// poll()  -- fresh connection each time, retry once on error
// ---------------------------------------------------------------------------

bool DeviceWorker::poll() {
    try {
        // Fresh device each poll (proven pattern -- persistent connections unreliable)
        auto device = std::make_unique<nanotuya::TuyaDevice>(config_.tuya_config);
        auto data = device->queryStatus();

        // Retry once on session/key errors (matches Python is_error retry)
        if (!data.has_value()) {
            std::this_thread::sleep_for(std::chrono::seconds(1));
        ...
        // queryStatus() returns the DPS object directly (e.g. {"1":true,"3":255})
...
// ---------------------------------------------------------------------------
// drainCommands()  -- process all queued commands with retry
// ---------------------------------------------------------------------------
...
void DeviceWorker::drainCommands() {
    ...
    for (auto& qc : cmds) {
        ...
        // Optimistic update -- give HA instant feedback
...
// ---------------------------------------------------------------------------
// executeCommand()  -- fresh connection, parse JSON, set DPS values
// ---------------------------------------------------------------------------
...
bool DeviceWorker::executeCommand(const Json::Value& cmd) {
    try {
        ...
        // Read back actual state after brief settle
...
// ---------------------------------------------------------------------------
// publishState()  -- parse DPS, diff against last, call publish_cb_
// ---------------------------------------------------------------------------
...
void DeviceWorker::publishState(const Json::Value& dps) {
    ...
    // Compare with last state -- only publish if changed
    {
        std::lock_guard<std::mutex> lk(state_mutex_);
    ...
    // Log changes
    {
        ...
        if (!last_state_.isNull()) {
            // Log changed keys (skip colour_data)
...
// ---------------------------------------------------------------------------
// publishOptimistic()  -- instant HA feedback before device confirms
// ---------------------------------------------------------------------------
